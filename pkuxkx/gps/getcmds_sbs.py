# -*- coding: utf-8 -*-
import sys
from mudsearch import MudRoomSearchProblem,MyNode,lbylines,getExits
import Queue
def breadthFirstTCSearch(problem): 
    ...
    while True:
        ...
        for successor in problem.getTCSuccessors(node.status):
...
def walk_stepbystep(ref_rid,lines,goal_rid):
    #return action and next ref_rid
    if len(ref_rid)==0 or ref_rid == "0":
        ref_rid = goal_rid
    rid = lbylines(ref_rid,lines)
    if rid == 0:        
        return ("0","0")
    else:
        p = MudRoomSearchProblem(rid,goal_rid)
...
#    print walk_stepbystep(ref_rid,lines,goal_rid)
    ...
    goal_rid = args[3].decode("gb18030").encode("utf8");
    (nextaction,nextrid) = walk_stepbystep(ref_rid,lines,goal_rid)
    print nextrid
