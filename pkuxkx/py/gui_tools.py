...
class TextBox:
    def __init__(self,screen,rect, font=None,font_color= (255, 255, 255),callback=None):
        """
        :param rect:文本框rect
    ...
    def draw(self):
        ...
        self.screen.blit(text_surf, (self.left,self.top))
        #draw a flash _ after text
        if self.focused:
            cmd_flash_img = pygame.image.load(cfg_gui.IMAGE_PATHS['cmd_flash'])
            self.screen.blit(cmd_flash_img,(self.left+text_surf.get_width()+1,self.top))
